...
public class UploadUtil {
	...
	// 上传图片
	public static Result<String> uploadImage(String userId, MultipartFile file) {
		...
		File targetFile = new File(Common.FMTP_PATH + path, fileName);
	...
	public static Result<String> uploadFile(String userId, MultipartFile file) {
		...
		File targetFile = new File(Common.UP_LOAD_PATH + path, fileName);
		File ytFile = new File(Common.UP_LOAD_PATH + path, "yt" + fileName);
		if(!targetFile.exists()) {
			targetFile.getParentFile().mkdirs();
		}
		if(!ytFile.exists()){
			ytFile.getParentFile().mkdirs();
		}
		// 保存
		try {
			file.transferTo(targetFile);
			...
			if(type != null && ".jpg.JPG".contains(type)){
				copyFile(Common.UP_LOAD_PATH + path+ fileName, Common.UP_LOAD_PATH + path+ "yt" + fileName, true);
...
//		        out.close();
	...
	// 上传文件
	public static Result<String> uploadString2File(String userId, String content, String type) {
		Result<String> res = new Result<String>();
		// 存储路径
		String path = DateUtil.nowYMD() + "/" + userId + "-/";
		String fileName = DateUtil.nowLong() + type;
		File targetFile = new File(Common.UP_LOAD_PATH + path, fileName);
		if(!targetFile.exists()) {
			targetFile.getParentFile().mkdirs();
		}
	...
	public static Result<String> uploadProject(MultipartFile file) {
		...
		// 存储路径
		String fileName = file.getOriginalFilename();
		File targetFile = new File(Common.UP_LOAD_PATH, fileName);
